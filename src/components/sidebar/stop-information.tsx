...
// Unused imports left for future features
        ...
        {isLoadingArrivals ? (
          ...
          <ScrollArea className="h-[150px] pr-2">
            <ul className="space-y-2.5">
              {arrivals.map((arrival: ObaArrivalDeparture, index: number) => (
                <li key={`${arrival.tripId}-${arrival.scheduledArrivalTime}-${index}`} className="p-2 rounded-md border bg-card/50">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      {arrival.status && (
                        <span 
                          ...
                          title={`Status: ${arrival.status.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (l: string) => l.toUpperCase())}`}
                        ></span>
                      )}
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    {arrival.status && <p className="leading-tight break-words">Status: {arrival.status.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (l: string) => l.toUpperCase())}</p>}
