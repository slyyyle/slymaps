...
export interface ObaVehicleLocation {
  ...
  predicted?: boolean; // Whether this vehicle has real-time data
  scheduleDeviation?: number; // seconds ahead/behind schedule (negative = early)
  // Status information for stop tracking
  closestStopId?: string | null;
  closestStopTimeOffset?: number | null; // seconds until/since closest stop
  nextStopId?: string | null;
  nextStopTimeOffset?: number | null; // seconds until next stop
  distanceAlongTrip?: number | null; // distance traveled along trip
  totalDistanceAlongTrip?: number | null; // total trip distance
}
...
export interface ObaSituation {
  ...
  affects?: {
    ...
    vehicleJourneys?: Array<{
      datedVehicleJourneyRef: { datedVehicleJourney: string };
      lineRef: string;
    }>;
  };
}
